...
<template>
    <div v-if="data.type === 'perDay' || data.type === 'perMonth' || data.type === 'perYear' " class="lg:w-2/3 w-full mx-auto overflow-auto">
        <table class="table-auto w-full text-left whitespace-no-wrap">
            <thead>
                <tr>
                    <th class="px-4 py-3 title-font tracking-wider font-medium text-gray-900 text-sm bg-gray-100 rounded-tl rounded-bl">년월일</th>
                    <th class="px-4 py-3 title-font tracking-wider font-medium text-gray-900 text-sm bg-gray-100">금액</th>
                </tr>
            </thead>
            <tbody>
    ...
    <div v-if="data.type === 'decile'" class="lg:w-2/3 w-full mx-auto overflow-auto">
        <table class="table-auto w-full text-left whitespace-no-wrap">
            <thead>
                <tr>
                    <th class="px-4 py-3 title-font tracking-wider font-medium text-gray-900 text-sm bg-gray-100 rounded-tl rounded-bl">그룹</th>
                    <th class="px-4 py-3 title-font tracking-wider font-medium text-gray-900 text-sm bg-gray-100">평균</th>
                    <th class="px-4 py-3 title-font tracking-wider font-medium text-gray-900 text-sm bg-gray-100">합계금액</th>
                    <th class="px-4 py-3 title-font tracking-wider font-medium text-gray-900 text-sm bg-gray-100">구성비</th>
                </tr>
            </thead>
            <tbody>
                <tr v-for="item in data.data" :key="item.date">
    ...
    <div v-if="data.type === 'rfm'" class="lg:w-2/3 w-full mx-auto overflow-auto">
        <h2 class="text-center text-2xl my-4">RFM 분석결과</h2>
        <div class="my=4 text-center">합계: {{ data.totals }} 인</div>
        <h3 class="text-center text-2xl my-4">RFM랭크별 인수</h3>
        <table class="table-auto w-full text-left whitespace-no-wrap">
            <thead>
                <tr>
        ...
        <h3 class="text-center text-2xl my-4">R과F의 집계표</h3>
        <table class="table-auto w-full text-left whitespace-no-wrap">
            <thead>
                <tr>
